	...
	if(isset($_POST['PGUpload'])) {
		...
		if(isset($theFile['small'])) {
			...
			if(is_array($gogo)) {
				...
				echo '<b><h3>Success File has been uploaded!... </h3></b></br><a href="/forum/gallery" title="Go Back To Main Gallery to view image">Go Back to main Gallery</a>....</br>';
			}
		} 
		else {
...
case 'upload':
	...
	<div class="Info">
		<form action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post" enctype="multipart/form-data">
			<div>
				<input type="hidden" name="PGWhichFolder" value="<?php echo $folder; ?>" />
				File - <input type="file" name="image"/><br />
				Picture Caption - <input class="InputBox" type="text" size="20" name="PGCaption" />
				<div class="Buttons">
					<input id="Form_Save" name="PGUpload" value="Upload New Picture" class="Button" type="submit">
				</div>
			</div>
		</form>
	</div>
...
default:
	...
	<div class="Info">
		...
		<form action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post">
			...
			<div class="Buttons">
				<input id="Form_Save" name="PGFolder" value="Create New Album" class="Button" type="submit">
			</div>
		</form>
	</div>
...
function uploadNresizeGallery($file, $dir) {
	...
	if($countinue == true) {
		if($file['image']['error']) {
			$errors[] = "Sorry nothing was uploaded..or"; echo '<h21>Sorry nothing was uploaded</h21>';
		}
		if($file['image']['size'] > $maxSize) {
			$errors[] = "The image size you uploaded was too big .... or";
		}
		if($file['image']['type'] != "image/jpeg" && $file['image']['type'] != "image/pjpeg" && $file['image']['type'] != "image/gif" && $file['image']['type'] != "image/png" && $file['image']['type'] != 'image/x-png') {
			$errors[] = "Invalid Image format, supported image types are .jpg (.jpeg), .gif, and .png. -- ".$file['image']['type'];
		}
